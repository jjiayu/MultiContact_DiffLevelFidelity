...
def Humanoid_FourPoint_AngularMomentum():
    ...
    for stepIdx in range(Nstep):
        pxtemp = ca.MX.sym('px'+str(stepIdx+1))
        px.append(pxtemp)
        px_lb.append(np.array([0]))
        px_ub.append(np.array([1.5]))

        pytemp = ca.MX.sym('py'+str(stepIdx+1))
        py.append(pytemp)
        py_lb.append(np.array([-1]))
        py_ub.append(np.array([1]))

        #   Foot steps are all staying on the ground
        pztemp = ca.MX.sym('pz'+str(stepIdx+1))
        pz.append(pztemp)
    ...
    for n_phase in range(Nphase):
        Tstemp = ca.MX.sym('Ts'+str(n_phase+1))
        Ts.append(Tstemp)
    ...
    #               print('No implementation yet')
                ...
                if StepCnt == 0:#First Step
                    ...
                    g.append(K_CoM_Right@(CoM_k-ca.DM(PR_init))-ca,DM(k_CoM_Right))
                    glb.append(np.full((len(k_CoM_Right),),-np.inf))
    ...
    #return solver, DecisionVars_init, DecisionVars_lb, DecisionVars_ub, glb, gub
